...
const EmbeddingVisualization: React.FC = () => {
  ...
  const [colorMode, setColorMode] = useState<"rainbow" | "similarity">(
    "rainbow",
  );
  ...
  useEffect(() => {
    ...
    const negativeZPlane = new THREE.Mesh(
      planeGeometry,
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const newDistance = Math.max(
        10,
        Math.min(200, distance + event.deltaY * 0.1),
      );
  ...
  const generateEmbeddings = useCallback(async () => {
    ...
    try {
      ...
      const tokenizer = await AutoTokenizer.from_pretrained(model_id, {
        progress_callback: (data: any) => {
          if (data.status === "downloading") {
            setLoadingProgress(
              `Downloading tokenizer: ${Math.round(data.progress)}%`,
            );
          }
        },
      });
      ...
      const inputs = await tokenizer(prefixedTexts, {
        padding: true,
        truncation: true,
      });
      ...
      console.log(
        ...
        processedEmbeddings[0]?.slice(0, 5),
      );
      console.log(
        "First embedding magnitude:",
        Math.sqrt(
          processedEmbeddings[0]?.reduce(
      ...
      console.log("Embedding value ranges:", {
        min: minVal,
        max: maxVal,
        mean: meanVal,
      });
      ...
      console.log(
        ...
        reducedEmbeddings.length,
        "x",
        reducedEmbeddings[0]?.length,
      );
      ...
      console.log("Reduced embedding ranges:", {
        min: minReduced,
        max: maxReduced,
        range: rangeReduced,
      });
  ...
  const updateVisualization = useCallback(
    (points: Point3D[]) => {
      ...
      console.log(
        "Creating geometry with positions array size:",
        positions.length,
      );
    ...
    <div className="mx-auto w-full max-w-6xl space-y-8 p-6">
      ...
      <div className="space-y-4 text-center">
        ...
        <p className="mx-auto max-w-3xl text-lg text-gray-600 dark:text-gray-300">
          Transform text into 3D visualizations using AI embeddings and UMAP
          dimensionality reduction. See how similar texts cluster together in 3D
          space, including the negative Z axis!
        </p>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700"
          ...
          <button
            ...
            className="w-full rounded-lg bg-green-500 px-6 py-3 font-medium text-white transition-colors hover:bg-green-600 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isLoading
              ? "Generating Embeddings..."
              : `Generate 3D Visualization (${texts.length} texts)`}
          </button>

          {isLoading && (
            <div className="rounded-lg bg-gray-50 p-3 text-sm text-gray-600 dark:bg-gray-700 dark:text-gray-400">
              {loadingProgress}
            </div>
          )}
        </div>
          ...
          <div className="mt-6">
            ...
            <div className="max-h-80 space-y-2 overflow-y-auto">
              {texts.map((text, index) => (
                <div
                  key={index}
                  className="flex items-center gap-3 rounded-lg bg-gray-50 p-3 text-sm dark:bg-gray-700"
                >
                  <span className="flex-1">{text}</span>
                  <button
                    onClick={() => removeText(index)}
                    className="rounded px-2 py-1 text-lg font-bold text-red-500 hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-900/20"
                  >
                    ×
                  </button>
                </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
        ...
        <div className="space-y-6 text-sm text-gray-600 dark:text-gray-300">
          <div className="space-y-2">
            ...
            <p>
              Each text is converted into a high-dimensional vector using
              Google's EmbeddingGemma model, capturing semantic meaning.
            </p>
          </div>
          <div className="space-y-2">
            <h3 className="font-semibold text-gray-900 dark:text-white">
            ...
            <p>
              UMAP reduces the high-dimensional embeddings to 3D coordinates
              while preserving the relationships between texts.
            </p>
          </div>
          <div className="space-y-2">
            <h3 className="font-semibold text-gray-900 dark:text-white">
            ...
            <p>
              Points are positioned in 3D space where similar texts cluster
              together. Click points to see similarity-based coloring!
            </p>
            <p>
              The visualization loads entirely in your browser using WebGL and
              WebAssembly - no server required. All the AI model inference
              happens locally, keeping your text private.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
        <div className="border-b border-gray-200 p-4 dark:border-gray-700">
          ...
          {selectedPoint && (
            <div className="mt-3 rounded-lg bg-blue-50 p-3 text-sm dark:bg-blue-900/20">
              ...
              <div className="mb-2 italic text-blue-700 dark:text-blue-200">
                "{selectedPoint.text}"
              </div>
              <button
                onClick={() => {
        ...
        <div className="relative" style={{ height: "600px" }}>
          ...
          {hoveredPoint && (
            <div
              className="pointer-events-none absolute z-10 max-w-xs rounded bg-gray-900 px-3 py-2 text-sm text-white shadow-lg dark:bg-gray-100 dark:text-gray-900"
              style={{
        ...
        <div className="border-t border-gray-200 p-4 text-xs text-gray-500 dark:border-gray-700 dark:text-gray-400">
          <p>
            💡 <strong>Tips:</strong> Click points to see similarity colors •
            Scroll to zoom, drag to rotate • Green plane = Z=0, Red plane =
            negative Z
          </p>
        </div>
      </div>
